...
class Requests:
    ...
    def post_req(name,email):        
        return requests.post(BASE_URL,headers=header, data = {"name": name, "email": email, "gender": "male", "status": "active"})

    def put_req(user_id,name,email):
        return requests.put('{0}/{1}'.format(BASE_URL, user_id),headers=header, data = {"name": name, "email": email, "gender": "male", "status": "active"})
