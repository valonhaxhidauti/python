...
class CurrencyConverter:
    ...
    def set_admin_convertion_withPass(self,rate, password, user_admin):
        if user_admin.get_pass() != password:
            raise Exception("Nuk mund ta qasni metoden e re te konvertimit pasi keni dhene gabim passowrdin e adminit")
        self.conversion_rate = rate    
...
user_admin = CurrencyConverter(user1,password1,True) # Inicializojme nje user admin
user_normal = CurrencyConverter(user2,password2,False) # Inicializojme nje user normal
...
print(euro,'euro me konvertimin nga useri admin me metoden e pare:', euro_convert_first, 'dollare')

euro_convert_first_user = user_normal.convert_eur_usd(float(euro))
print('Konvertimi i njejte nga nje user normal me metoden e pare:',euro_convert_first_user,'dollare')
...
print(euro,'euro me konvertimin e metodes se dyte jane:', euro_convert_second, 'dollare')



try:
    user_normal.set_admin_convertion(3) # Tentojme te bejme nje konvertim me nje norme(ne kete rast 3) nga nje user i thjeshte
    euro_convert_second_user = user_normal.convert_eur_usd(float(euro))
    print('Konvertimi i userit normal:',euro_convert_second_user,'dollare') # Kjo linje nuk printohet kurre pasi qe useri normal nuk ka qasje ne metoden e dyte
except Exception as ex:
...
try:
    ...
    euro_third = user_normal.convert_eur_usd(float(euro))   #Bejme konvertimin e ri te eurove nga nje user i thjeshte me norme te re 
    print('Konvertimi i ri me norme',rate,'i',euro,'eurove eshte:',euro_third,'dollare')
except Exception as ex:
    print(ex)
